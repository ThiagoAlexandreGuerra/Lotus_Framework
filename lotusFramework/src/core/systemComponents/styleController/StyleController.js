...
export default class StyleController {
    ...
    getHeight(){
        const height = parseFloat(this._styleConfig.height.replace("px", ""));
        return height;
    }
    getWidth(){
        const width = parseFloat(this._styleConfig.width.replace("px", ""));
        return width;
    }
    getTop(){
   ...
   _set(prop, styleProp, value) {
    ...
    _setProperty(prop , propertyProp , value , isAddRoutine = false){
        ...
        if(isAddRoutine && Array.isArray(this[prop]) &&  this._propertyConfig[propertyProp]){
            ...
            this[prop].add(value);
            this._propertyConfig[propertyProp].add(value);
